...
| **Step size limit**         | Step description implies modifying >5 files (unconditional)                                                                            | Split into substeps grouped by module or concern. Each substep gets its own `style_card` field and "Run style validator on changed files" instruction. The brainstorm usually names file counts — use those to detect oversized steps during planning.                                          |
