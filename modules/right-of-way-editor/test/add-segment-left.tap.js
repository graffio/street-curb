import t from 'tap'
import curbReducer, { addSegmentLeft } from '../src/store/curbStore.js'
...
t.test('addSegmentLeft Redux action', async t => {
    await t.test('Given a segment with sufficient length', async t => {
        await t.test('When dispatching addSegmentLeft for current segment', async t => {
            ...
            const result = curbReducer(initialState.curb, action)
            ...
            t.equal(result.segments[0].type, 'Parking', 'Then the new segment has the default type')
            t.equal(result.segments[1].type, 'Parking', 'Then the original segment type is preserved')
        })
    ...
    await t.test('Given segments with insufficient space', async t => {
        await t.test('When trying to split a segment that is too small', async t => {
            ...
            t.same(result.segments, initialSegments, 'Then the segments remain unchanged')
        })
    ...
    await t.test('Given default desired length', async t => {
        await t.test('When not specifying desired length', async t => {
            ...
            t.equal(result.segments.length, 2, 'Then two segments exist')
            t.equal(result.segments[0].length, 10, 'Then the first segment has the default length of 10')
            t.equal(result.segments[1].length, 20, 'Then the second segment has the remaining length')
        })
    })
})
