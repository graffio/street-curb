...
// ABOUTME: Variant-agnostic — builds a patch and reconstructs via constructor.from()
...
const buildCombinedFilter = (baseFilter, chipFilters) => {
...
const buildChipFilters = ({ selectedCategories, selectedAccounts, filterQuery }, accts) => {
    const result = []
...
const buildChipPatch = (ir, { asOfDate, dateRange, groupBy, ...rest }, accts) => {
    ...
    const patch = {}
    if (chipFilters.length > 0) patch.filter = buildCombinedFilter(ir.filter, chipFilters)
...
const applyChipFilters = (ir, chipState, accts) => {
    ...
    const patch = buildChipPatch(ir, chipState, accts)
...
const MergeChipFilters = { applyChipFilters, buildCombinedFilter, buildChipFilters, buildChipPatch }

export { MergeChipFilters }
