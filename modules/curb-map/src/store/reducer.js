...
const rootReducer = (state = initialState, { type, payload: action }) => {
    ...
    if (Action.is(action)) return action.match({
        ...
        OrganizationSynced: () => ({ ...state, currentOrganization: action.organization, currentProjectId: action.organization.defaultProjectId }),
        BlockfacesSynced: () => ({ ...state, blockfaces: LookupTable(action.blockfaces, Blockface, 'id') }),
