...
import { Entry, Tag } from '../src/types/index.js'
...
test('Tags Repository', t => {
    t.test('Given a fresh database', t => {
        ...
        t.test('When I insert a basic tag', t => {
            const tagEntry = Entry.Tag.from({ name: 'Important', color: '#FF0000', description: 'High priority items' })
        ...
        t.test('When I insert a tag with minimal data', t => {
            const tagEntry = Entry.Tag.from({ name: 'Work' })
    ...
    t.test('Given a database with existing tags', t => {
        ...
        const tags = [
            Entry.Tag.from({ name: 'Important', color: '#FF0000', description: 'High priority' }),
            Entry.Tag.from({ name: 'Work', color: '#0000FF', description: 'Work-related items' }),
            Entry.Tag.from({ name: 'Personal', color: '#00FF00' }),
        ]
        ...
        t.test('When I import additional tags', t => {
            const newTags = [
                Entry.Tag.from({ name: 'Urgent', color: '#FF6600', description: 'Very urgent items' }),
                Entry.Tag.from({ name: 'Review', description: 'Needs review' }),
            ]
