...
test('buildSecurityLookup', async t =>
    t.test('Given a database with securities', async t => {
        ...
        t.test('When building lookup', async t => {
            ...
            t.test('Then bySymbol contains uppercase signatures with orphan info', async t => {
                const entry = lookup.bySymbol.get('AAPL')
                t.equal(entry.id, 'sec_000000000111')
                t.equal(entry.orphanedAt, null)
            })

            t.test('Then byName contains lowercase signatures with orphan info', async t => {
                const entry = lookup.byName.get('vanguard total stock')
                t.equal(entry.id, 'sec_000000000222')
                t.equal(entry.orphanedAt, null)
            })
        })
    }))
