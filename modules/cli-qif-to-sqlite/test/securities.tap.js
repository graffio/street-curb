...
import { Entry, Security } from '../src/types/index.js'
...
test('Securities Repository', t => {
    t.test('Given a fresh database', t => {
        ...
        t.test('When I insert a basic security', t => {
            const securityEntry = Entry.Security.from({
                name: 'Apple Inc.',
                symbol: 'AAPL',
                type: 'Stock',
                goal: 'Growth',
        ...
        t.test('When I insert a security with minimal data', t => {
            const securityEntry = Entry.Security.from({ name: 'Microsoft Corporation' })
    ...
    t.test('Given a database with existing securities', t => {
        ...
        const securities = [
            Entry.Security.from({ name: 'Apple Inc.', symbol: 'AAPL', type: 'Stock' }),
            Entry.Security.from({ name: 'Microsoft Corporation', symbol: 'MSFT', type: 'Stock' }),
            Entry.Security.from({ name: 'Tesla Inc.', symbol: 'TSLA', type: 'Stock' }),
        ]
        ...
        t.test('When I import additional securities', t => {
            const newSecurities = [
                Entry.Security.from({ name: 'Google LLC', symbol: 'GOOGL', type: 'Stock' }),
                Entry.Security.from({ name: 'Amazon.com Inc.', symbol: 'AMZN', type: 'Stock' }),
            ]
