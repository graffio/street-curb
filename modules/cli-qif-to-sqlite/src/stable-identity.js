...
// @sig findBySignature :: (Database, String, String) -> String | undefined
...
// Lookup stable identity including orphaned ones, returns {id, orphanedAt} or undefined
// @sig findBySignatureIncludingOrphaned :: (Database, String, String) -> {id, orphanedAt} | undefined
const findBySignatureIncludingOrphaned = (db, entityType, signature) => {
    ...
    const row = stmt.get(entityType, signature)
    return row ? { id: row.id, orphanedAt: row.orphanedAt } : undefined
}
...
// @sig persistIdentity :: (Database, {id, entityType, signature}) -> void
const persistIdentity = (db, { id, entityType, signature }) => {
    const stmt = E.persistCachedStatement(
        db,
        'insertIdentity',
...
// @sig setOrphaned :: (Database, String) -> void
const setOrphaned = (db, stableId) => {
    const stmt = E.persistCachedStatement(
        db,
        'markOrphaned',
...
// @sig resetOrphanStatus :: (Database, String) -> void
const resetOrphanStatus = (db, stableId) => {
    const stmt = E.persistCachedStatement(
        db,
        'restore',
...
const updateLastModified = (db, stableId) => {
    const stmt = E.persistCachedStatement(
        db,
        'touch',
