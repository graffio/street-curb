...
const lineGroupsToEntries = lineGroups => {
    ...
    const byEntryType = entry => {
        if (Entry.Account.is(entry))               return 'accounts'
        if (Entry.Category.is(entry))              return 'categories'
        if (Entry.Class.is(entry))                 return 'classes'
        if (Entry.Security.is(entry))              return 'securities'
        if (Entry.TransactionBank.is(entry))       return 'bankTransactions'
        if (Entry.TransactionInvestment.is(entry)) return 'investmentTransactions'
        if (Entry.Payee.is(entry))                 return 'payees'
        if (Entry.Price.is(entry))                 return 'prices'
        if (Entry.Tag.is(entry))                   return 'tags'
        return 'others'
    }
