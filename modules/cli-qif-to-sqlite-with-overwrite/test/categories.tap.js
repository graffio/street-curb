// ABOUTME: Tests for category database operations
// ABOUTME: Validates category insert, find, and query functionality
...
import { Category, QifEntry } from '../src/types/index.js'
...
test('Categories Repository', t => {
    t.test('Given a fresh database', t => {
        ...
        t.test('When I insert a basic category', t => {
            const categoryEntry = QifEntry.Category.from({
                name: 'Groceries',
        ...
        t.test('When I insert a category with minimal data', t => {
            const categoryEntry = QifEntry.Category.from({ name: 'Utilities' })
    ...
    t.test('Given a database with existing categories', t => {
        ...
        const categories = [
            QifEntry.Category.from({ name: 'Groceries', description: 'Food items' }),
            QifEntry.Category.from({ name: 'Transportation', description: 'Gas and transit' }),
            QifEntry.Category.from({ name: 'Entertainment', description: 'Movies and dining' }),
        ]
        ...
        t.test('When I import additional categories', t => {
            const newCategories = [
                QifEntry.Category.from({ name: 'Healthcare', description: 'Medical expenses' }),
                QifEntry.Category.from({ name: 'Education', description: 'School and training' }),
            ]
