// ABOUTME: Tests for account database operations
// ABOUTME: Validates account insert, find, and query functionality
...
import { Account, QifEntry } from '../src/types/index.js'
...
const validEntryAccount = QifEntry.Account.from({
...
const validEntryAccount2 = QifEntry.Account.from({
...
test('Account Repository Tests', t => {
    t.test('Given an invalid account object', t => {
        t.test('When inserting into something into the database that is NOT an QifEntry.Account', t => {
            const db = createTestDatabase()
            t.throws(
                () => insertAccount(db, invalidAccount),
                /Expected QifEntry.Account; found: {"name":"Invalid Account","type":"Bank"}/,
    ...
    t.test('Given a valid QifEntry.Account', t => {
