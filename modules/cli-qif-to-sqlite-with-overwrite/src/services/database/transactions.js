...
import { QifEntry, Transaction } from '../../types/index.js'
 ...
 * @sig insertBankTransaction :: (Database, QifEntry.TransactionBank, Account) -> String
 */
const insertBankTransaction = (db, transactionEntry, account) => {
    ...
    if (!QifEntry.TransactionBank.is(transactionEntry))
        throw new Error(`Expected QifEntry.TransactionBank; found: ${JSON.stringify(transactionEntry)}`)
 ...
 * @sig insertInvestmentTransaction :: (Database, QifEntry.TransactionInvestment, Account, Security?) -> String
 */
const insertInvestmentTransaction = (db, transactionEntry, account, security = null) => {
    if (!QifEntry.TransactionInvestment.is(transactionEntry))
        throw new Error(`Expected QifEntry.TransactionInvestment; found: ${JSON.stringify(transactionEntry)}`)
 ...
 * @sig importBankTransactions :: (Database, [QifEntry.TransactionBank], [Account]) -> [String]
 */
const importBankTransactions = (db, transactions, accounts) => {
    /*
     * Handle bank transaction import with error handling
     * @sig handleBankTransactionImport :: QifEntry.TransactionBank -> String
     */
 ...
 * @sig importInvestmentTransactions :: (Database, [QifEntry.TransactionInvestment], [Account], [Security]) -> [String]
 */
const importInvestmentTransactions = (db, transactions, accounts, securities) => {
     ...
     * @sig handleInvestmentTransactionImport :: QifEntry.TransactionInvestment -> String
     */
