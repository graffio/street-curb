...
import { Category, QifEntry } from '../../types/index.js'

/*
 * Insert category into database (dedupes on collision)
 * @sig insertCategory :: (Database, QifEntry.Category) -> String
 */
const insertCategory = (db, categoryEntry) => {
    const generateId = n => `cat_${hashFields({ name: n })}`

    if (!QifEntry.Category.is(categoryEntry))
        throw new Error(`Expected QifEntry.Category; found: ${JSON.stringify(categoryEntry)}`)
 ...
 * @sig importCategories :: (Database, [QifEntry.Category]) -> [String]
 */
