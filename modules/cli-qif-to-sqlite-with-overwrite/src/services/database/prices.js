...
import { QifEntry, Price } from '../../types/index.js'
 ...
 * @sig insertPrice :: (Database, QifEntry.Price, Security) -> String
 */
const insertPrice = (db, priceEntry, security) => {
    if (!QifEntry.Price.is(priceEntry)) throw new Error(`Expected QifEntry.Price; found: ${JSON.stringify(priceEntry)}`)
 ...
 * @sig importPrices :: (Database, [QifEntry.Price], [Security]) -> [String]
 */
const importPrices = (db, prices, securities) => {
    ...
    // @sig processPriceEntry :: QifEntry.Price -> void
