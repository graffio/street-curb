...
// COMPLEXITY: export-structure — Legacy export name preserved for compatibility
// COMPLEXITY-TODO: lines — Pre-existing debt, QIF parsing inherently complex (expires 2026-07-01)
// COMPLEXITY-TODO: functions — Pre-existing debt, QIF parsing inherently complex (expires 2026-07-01)
...
// COMPLEXITY-TODO: max-line-length — Pre-existing debt, QIF parsing (expires 2026-07-01)
...
const lineGroupToEntry = (currentContext, currentAccount, lineGroup) => {
    ...
    const transactionBankReducer = (acc, line) => {
        ...
        const endSplit = value => {
            ...
            return update(index, QifSplit(parseAmount(value), category, memo), acc.splits)
        }
        const [key, value] = [line[0], line.slice(1)]
    ...
    // @sig toPrices :: () -> [QifEntry.Price]
    const toPrices = () => {
        const toPrice = line => {
            const [symbol, priceValue, date] = line.split(',').map(v => v.trim().replace(/"/g, ''))
            if (!priceValue) return null
            return QifEntry.Price.from({ symbol, price: parseAmount(priceValue), date: standardizeDate(date) })
        }
