...
import t from 'tap'
import { Factories as FS } from '../src/lib/shared/factories.js'
...
t.test('Given withExemptions wrapper', t => {
    ...
    t.test('When the source has no COMPLEXITY comment', t => {
        const result = FS.withExemptions('test-rule', mockCheckFn, null, 'const x = 1', '/test.js')
    ...
    t.test('When the source has a permanent COMPLEXITY exemption', t => {
        ...
        const result = FS.withExemptions('test-rule', mockCheckFn, null, code, '/test.js')
    ...
    t.test('When the check function returns no violations', t => {
        const noViolationFn = () => []
        const result = FS.withExemptions('test-rule', noViolationFn, null, 'const x = 1', '/test.js')
