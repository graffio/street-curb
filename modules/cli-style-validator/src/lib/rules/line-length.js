...
const nodeHasPrettierIgnore = (node, lines) => {
    ...
    const precedingLines = A.getPrecedingCommentLines(lines, nodeStartLine)
...
const P = { hasPrettierIgnore, isBoundaryLine, nodeHasPrettierIgnore, shouldReportLine }
...
const T = { lineRange, getNodeLineNumbers }
 ...
 */
const collectIgnoredLines = (lines, node) => (P.nodeHasPrettierIgnore(node, lines) ? T.getNodeLineNumbers(node) : [])
...
const A = { getPrecedingCommentLines, collectIgnoredLines, buildIgnoredLinesSet }
