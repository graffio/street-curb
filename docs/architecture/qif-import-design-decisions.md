...
**Previous design (rejected):** Two-level system with content-hash IDs (`acc_${hash}`) in base tables mapped to stable
IDs (`sac_00001`). This added unnecessary complexity—the hash-based IDs served no purpose that stable IDs don't also
serve.
...
**Decision:** Import order is: accounts → categories → tags → securities → transactions (with splits) → prices → lots →
lot allocations.
...
**Decision:** If both old and new security have symbols, match by symbol only. Fall back to name only if either lacks
symbol.
...
**Decision:** If N transactions have identical signatures and N stable IDs expect them, pair them 1:1 without caring
which maps to which.
...
**Alternative rejected:** Aggressive normalization (strip punctuation, numbers) — risks matching different transactions
silently.
...
**Decision:** Import uses INSERT for new entities, UPDATE for existing. Orphaned entities are soft-deleted (`orphanedAt`
...
**Rationale:** A partial replacement would require maintaining two codebases and deciding which to use. Better to build
once correctly.
...
**Exception:** FIFO lot tracking is extracted to `import-lots.js` because it's algorithmically distinct (stateful
computation vs simple insert-or-match).
...
**Implementation:** After marking transaction orphaned, query splits by `transactionStableId` and mark those orphaned
too.
...
**Note:** Lots are computed, not imported from QIF. The signature ensures the same computation produces the same stable
ID.
...
**Why deferred:** Fungible pairing works adequately for duplicate transactions. Adding daily sequence adds complexity
without proven benefit. Can revisit if users report issues with duplicate transaction identity.
...
**Decision:** Lots are identified by their opening transaction, not by a separate stable ID. User overrides for lot
assignment are stored and survive reimports.

**Background:** QIF does not export lot information. When a user sells shares in Quicken and chooses specific lots (for
tax optimization), that choice is lost in the QIF export. We must compute lots from buy/sell transactions.
...
3. **User overrides:** For rare cases where user needs specific lot assignment, store overrides keyed to sell
   transaction
...
**Problem:** Position-based signatures (`transactionStableId|position|amount`) break when splits are inserted — all
subsequent splits shift positions and become orphaned.

**Split signature:** `transactionStableId|categoryStableId|amount`

**Duplicate handling:** If two splits have identical signatures (same category, same amount), they are fungible and
paired arbitrarily — same as duplicate transactions (D4).
...
**Future consideration:** If we ever need to sort Quicken dates alongside our datetimes, use a consistent convention (
e.g., market close in New York: `16:00 ET` / `21:00 UTC`).
...
**Key insight:** During import, we load all signatures into memory and match there. We don't query signatures via SQL.
The only SQL queries on stableIdentities are:
...
SET nextId = nextId + 1
WHERE entityType = 'Transaction' RETURNING nextId - 1 as currentId;
...
The 12-digit format matches existing web app regex patterns (`[a-f0-9]{12}`), ensuring compatibility without web app
changes. These IDs are used directly in base tables—there is no separate "entity ID" layer.
